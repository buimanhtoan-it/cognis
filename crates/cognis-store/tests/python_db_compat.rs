...
//!
//! Opens a `.cognis/uckg.db` **created by the Python engine** (the parity
//! oracle) and reads symbol / edge / FTS5 / vec without error, proving:
//!
...
//!
//! The fixture at `tests/fixtures/python_uckg.db` was generated by the Python
//! engine's `cognis.db` migration runner + write helpers, with sqlite-vec
//! forced off (the portable CI shape). It is checked in as a frozen
//! compatibility fixture; there is no Python toolchain to regenerate it.
...
fn fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/python_uckg.db")
}
...
fn open_temp_copy(tmp: &tempfile::TempDir) -> Database {
    ...
    let dst = tmp.path().join("python_uckg.db");
    fs::copy(&src, &dst).expect("copy fixture");
    Database::open(&dst).expect("open Python-built DB")
}

#[test]
fn opens_python_built_db_without_breaking_migrate() {
    let tmp = tempfile::tempdir().unwrap();
    let db = open_temp_copy(&tmp);

    // The Python engine wrote schema_version=1; our migration runner must treat
...
fn reads_symbol_edge_fts_vec_without_error() {
    ...
    for s in &symbols {
        s.validate()
            .expect("Python-written symbol passes core validation");
