...
| Memory footprint (indexd / mcpd) | record | no unbounded growth |
...
- Pillar 1: not yet at the bar (objective benchmark sample still small and
  Python-weighted) — the explicit next-cycle target. It governs public claims,
  not whether the tool ships.
