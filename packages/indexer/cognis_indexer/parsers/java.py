...
The ``package`` declaration is walked through but does not prefix the qualified
name (mirrors the ``ClassName.method`` convention used by the TypeScript parser).
...
Usage::
    ...
    parser = JavaParser()
    symbols = parser.parse(source_code, "src/main/java/auth/JwtValidator.java")

Requires ``tree-sitter>=0.22`` and ``tree-sitter-java>=0.21``.
"""
...
_COMMENT_TYPES: frozenset[str] = frozenset({"line_comment", "block_comment"})
...
def _lazy_load() -> Any:
    try:
        ...
        raise ImportError(
            "Java parser requires 'tree-sitter' and 'tree-sitter-java'. "
            "Install them with: pip install tree-sitter>=0.22 tree-sitter-java>=0.21"
        ) from exc
...
def _module_from_path(file_path: str) -> str:
    """Convert ``src/main/java/auth/JwtValidator.java`` → ``.../JwtValidator``."""
...
def _extract_docstring(node: Any, source_bytes: bytes) -> str | None:
    """Return the contiguous Javadoc / line-comment block directly above *node*."""
    parent = node.parent
    if parent is None:
        return None
...
class JavaParser:
    ...
    Instantiating this class eagerly loads ``tree-sitter-java``.
    ...
    def parse(self, source: str, file_path: str) -> list[ParsedSymbol]:
        """Parse Java *source* and return a flat list of symbols (never raises)."""
